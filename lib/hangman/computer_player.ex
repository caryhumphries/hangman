defmodule Hangman.ComputerPlayer do
  ...
  def play do
    game = Game.new_game
    word_length = Game.word_length(game)
    solver = %{
      candidate_words: load_words_of_length(word_length),
  ...
  def analyze_move(:good_guess, game, solver, guess) do
    IO.puts "#{inspect <<guess::utf8>>} was good"
  ...
  def analyze_move(:bad_guess, game, solver, guess) do
    IO.puts "#{inspect <<guess::utf8>>} was bad"
    IO.puts Game.word_as_string(game)
    IO.puts ""

    # current = solver.candidate_words
    # new_words = remove_words_with_letter(current, guess)
    # |> remove_words_not_matching_pattern(Game.word_as_string(game))
